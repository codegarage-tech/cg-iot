...
public class ClipboardHandler {
    ...
    public static void copyToClipboard(Context mContext, String data, String successMessage) {
    ...
    private static void copyToClipboardAboveHoneyComb(Context mContext, String data) {
        ClipboardManager clipboard = (ClipboardManager) mContext.getSystemService(Context.CLIPBOARD_SERVICE);//Get Clipboard Manager
        ClipData clip = ClipData.newPlainText("clipboard data", data);//Save plain text data to clip data
        clipboard.setPrimaryClip(clip);//set clip data as primary clip
    }

    //For below Honeycomb devices
    @SuppressWarnings("deprecation")
    private static void copyToClipboardBelowHoneyComb(Context mContext, String data) {
    ...
    //First Check SDK version because SDK below honeycomb has different method and above honeycomb is different
    public static String getDataFromClipboard(Context mContext, String successMessage) {
        String text = "";
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
