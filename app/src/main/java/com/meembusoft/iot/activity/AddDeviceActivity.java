...
import com.meembusoft.iot.adapter.AddDeviceViewPagerAdapter;
        ...
        btnPrevious.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!isFirstPage) {
                    ...
                    if (addDeviceViewPagerAdapter.getScreenType(vpAddDevice.getCurrentItem()) == AddDeviceScreenType.CONNECT_DEVICE) {
                        ConnectDeviceFragment connectDeviceFragment = (ConnectDeviceFragment) addDeviceViewPagerAdapter.getRegisteredFragment(vpAddDevice.getCurrentItem());
                        if (connectDeviceFragment != null) {
    ...
    private boolean isScreenVerified(int position) {
        ...
        switch (addDeviceViewPagerAdapter.getScreenType(position)) {
            ...
            case CONNECT_DEVICE:
                ConnectDeviceFragment connectDeviceFragment = (ConnectDeviceFragment) addDeviceViewPagerAdapter.getRegisteredFragment(position);
                if (connectDeviceFragment != null) {
                    if (connectDeviceFragment.isAllFieldsVerified()) {
                        Logger.d(TAG, TAG + ">>" + "wifi name and password are chose.");
                        // Stop wifi scanner
                        connectDeviceFragment.destroyTask();
                        return true;
                    } else {
                        Logger.d(TAG, TAG + ">>" + "wifi name and password are not chose.");
                    }
                } else {
                    Logger.d(TAG, TAG + ">>" + "connect device fragment is null.");
                }
                return false;
            case PREVIEW_SCREEN:
                return false;
