...
import com.meembusoft.iot.viewholder.StaggeredListViewHolder;

import static com.meembusoft.iot.util.AppUtil.chunkList;
...
public class StaggeredListAdapter extends RecyclerArrayAdapter<StaggeredListItem> {
    ...
    public StaggeredListAdapter(Context context) {
        super(context);
        mContext = context;
    }
    ...
    public BaseViewHolder OnCreateViewHolder(ViewGroup parent, int viewType) {
        switch (viewType) {
            case VIEW_TYPE_REGULAR:
                return new StaggeredListViewHolder(parent, mContext);
            default:
                throw new InvalidParameterException();
        }
    }

    public void setAllData(List<StaggeredItem> staggeredItems, int chunkSize) {
        if (staggeredItems != null && staggeredItems.size() > 0) {
            List<List<StaggeredItem>> chunkList = chunkList(staggeredItems, chunkSize);
    ...
    public void addData(List<StaggeredItem> staggeredItems, int chunkSize) {
        if (staggeredItems != null && staggeredItems.size() > 0) {

            List<StaggeredListItem> totalExistingItems = getAllData();
            if (totalExistingItems != null) {
                List<StaggeredItem> tempItems = new ArrayList<>();
...
//            }
    ...
    public void addData(StaggeredItem staggeredItems) {
//        if (staggeredItems != null && staggeredItems.size() > 0) {
